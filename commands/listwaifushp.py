...
@bot.command(description="|LISTING| Lists all of the waifus in the database for a given user relatively to their hp", aliases=("lwhp",))
async def listwaifushp(ctx: commands.Context, starting_index: int = 1):
    """
    Lists all the waifus in the database for a given user, in a paged list of 15 waifus per,
    relative to their hp.
    ...
    waifu_list: List[Document] = sorted(waifu_query, key=lambda x: x.content["max_hp"], reverse=True)
...
@listwaifushp.error
async def listwaifushp_error(error, ctx: commands.Context):
    """
    Handles errors for the listwaifushp command.
