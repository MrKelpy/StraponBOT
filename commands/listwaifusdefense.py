...
@bot.command(description="|LISTING| Lists all of the waifus in the database for a given user based on their physical defense",
             aliases=("lwdef",))
async def listwaifusdefense(ctx: commands.Context, starting_index: int = 1):
    """
    Lists all the waifus in the database for a given user, in a paged list of 15 waifus per, based
    on their physical defense.
    ...
    waifu_list: List[Document] = sorted(waifu_query, key=lambda x: x.content["physical_def"], reverse=True)
...
@listwaifusdefense.error
async def listwaifusdefense_error(error, ctx: commands.Context):
    """
    Handles errors for the listwaifusdefense command.
