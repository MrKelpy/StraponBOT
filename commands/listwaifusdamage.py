...
@bot.command(description="|LISTING| Lists all of the waifus in the database for a given user based on their physical damage",
             aliases=("lwdmg",))
async def listwaifusdamage(ctx: commands.Context, starting_index: int = 1):
    """
    Lists all the waifus in the database for a given user, in a paged list of 15 waifus per, based
    on their physical damage.
    ...
    waifu_list: List[Document] = sorted(waifu_query, key=lambda x: x.content["physical_dmg"])
...
@listwaifusdamage.error
async def listwaifusdamage_error(error, ctx: commands.Context):
    """
    Handles errors for the listwaifusdamage command.
