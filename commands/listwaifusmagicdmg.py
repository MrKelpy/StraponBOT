...
@bot.command(description="|LISTING| Lists all of the waifus in the database for a given user based on their magical damage",
             aliases=("lwmdmg",))
async def listwaifusmagicdmg(ctx: commands.Context, starting_index: int = 1):
    """
    Lists all the waifus in the database for a given user, in a paged list of 15 waifus per, based
    on their magical damage.
    ...
    waifu_list: List[Document] = sorted(waifu_query, key=lambda x: x.content["magical_dmg"], reverse=True)
...
@listwaifusmagicdmg.error
async def listwaifusmagicdmg_error(error, ctx: commands.Context):
    """
    Handles errors for the listwaifusmagicdmg command.
