...
@bot.command(description="|LISTING| Lists all of the waifus in the database for a given user based on their speed",
             aliases=("lwspd",))
async def listwaifusspeed(ctx: commands.Context, starting_index: int = 1):
    """
    ...
    waifu_list: List[Document] = sorted(waifu_query, key=lambda x: x.content["speed"], reverse=True)
...
@listwaifusspeed.error
async def listwaifusspeed_error(error, ctx: commands.Context):
    """
    Handles errors for the listwaifusspeed command.
